...
ALTER TABLE people 
  ...
  , ADD COLUMN employee BIT(1) NOT NULL DEFAULT 0
  , ADD COLUMN usermod      MEDIUMINT DEFAULT 2
  , ADD COLUMN datemod      DATETIME DEFAULT CURRENT_TIMESTAMP
  , ADD COLUMN useract      MEDIUMINT DEFAULT 1
  , ADD COLUMN dateact      DATETIME DEFAULT CURRENT_TIMESTAMP
...
UPDATE people
  SET email_addr = 'bvincelette@boxstore.com'
, password = MD5('ab')
, phone_pri = '204-845-1253'
, addr = '123 Main St'
, addr_code = 'N0M 2J6'
, addr_type_id = 2
, addr_info = '1234 STN A'
...
-- Update my info
UPDATE people
SET email_addr = 'coledorman@boxstore.com'
  , password = MD5('cd')
...
CREATE TABLE IF NOT EXISTS manufacturer(
    man_id    MEDIUMINT AUTO_INCREMENT -- PK
  ...
  -- FK geo_address_type
  , addr_type_id  TINYINT(4) NOT NULL 
  -- FK geo_towncity
  , tc_id         INT(11)
  , usermod      INT(11) DEFAULT 2
  , datemod      DATETIME DEFAULT CURRENT_TIMESTAMP
  , useract      INT(11) DEFAULT 1
  , dateact      DATETIME DEFAULT CURRENT_TIMESTAMP
  , active BIT(1) DEFAULT 1
  , CONSTRAINT man___PK PRIMARY KEY(man_id)
  , CONSTRAINT man___UK UNIQUE KEY(man_name)
...
VALUES         ( 'Boxstore Inc.', 1
               , '222-222-2222', '333-333-3333', '444-4444-4444'
...
CREATE VIEW IF NOT EXISTS manufacturer_people_reps AS (
    SELECT 
        ...
        , p.email_addr
        , p.phone_pri
        , p.addr AS street_addresss
        , p.addr_delivery AS po_box
        , gtc.tc_name AS city
        , grg.rg_name AS region
        , gco.co_name AS country
        , p.phone_pri AS phone
    FROM people p
    JOIN manufacturer m ON p.p_id_man = m.man_id
    JOIN geo_towncity gtc ON p.tc_id = gtc.tc_id
    JOIN geo_region grg ON gtc.rg_id = grg.rg_id
    JOIN geo_country gco ON grg.co_id = gco.co_id
    ORDER BY
...
-- THIS HOW JOIN
SELECT i.item_id, i.item_name, i.item_modelno, i.item_barcode, i.item_price
     , i.man_id, i.it_id, it.it_id, m.man_name
FROM item i
...
SELECT ip.ip_id, ip.item_id, ip.item_price, ip.ip_beg, ip.ip_end, i.item_id, i.item_name, i.item_modelno, i.item_barcode, i.item_price
     , i.man_id, i.it_id
FROM item_price ip
...
-- JOIN
SELECT o.order_id, o.order_no, o.order_date, o.p_id_cus, o.p_id_emp,
    e.first_name, e.last_name, ep.pe_employee_id
FROM orders o
JOIN people c ON o.p_id_cus = c.p_id
JOIN people e ON o.p_id_emp = e.p_id
JOIN people_employee ep ON e.p_id = ep.p_id;
...
-- JOIN
SELECT i.item_id, i.item_name, i.item_modelno, i.item_barcode, i.item_price
     , i.man_id, i.it_id, it.it_id, m.man_name, oi.oi_qty, oi.oi_qty * ip.item_price AS subtotal
     , GROUP_CONCAT(ii.ii_serialno,'')
FROM orders__item oi
...
CREATE VIEW IF NOT EXISTS receipt_00_header_company_info AS (
    SELECT 
         man_name AS company_name
        , phone_pri AS phone_number
        , phone_fax AS fax_number
        , addr_prefix || ' ' || addr AS company_address
        , addr_code
    FROM manufacturer
...
CREATE VIEW IF NOT EXISTS receipt_01_header_order_employee_customer AS (
    SELECT 
        o.order_id
      , o.order_date
      , o.p_id_emp AS employee_id
      , CONCAT(e.first_name, ' ', e.last_name) AS employee_name
      , o.p_id_cus AS customer_id
      , CONCAT(p.first_name, ' ', p.last_name) AS customer_name
      , p.email_addr
      , p.phone_pri AS phone_number
    FROM orders o
    JOIN people e ON o.p_id_emp = e.p_id  -- Join on p_id_emp (assuming FK)
    JOIN people p ON o.p_id_cus = p.p_id  -- Join on p_id_cus (assuming FK)
    WHERE p.employee = 0  -- Filter for non-employee in people
);
-- -------------------------------------------------------------------
...
CREATE VIEW IF NOT EXISTS receipt_10_body_line_items AS (
    SELECT 
        o.order_id
      , oi.oi_id AS item_id
      , CONCAT(i.item_name, ' (', oi.oi_qty, ')') AS description
      , oi.oi_qty AS quantity
      , i.item_price AS item_price
      , (oi.oi_qty * i.item_price) AS extended_price
      , oi.ip_override AS discount
      , (oi.oi_qty * i.item_price) - oi.ip_override AS final_price
      , GROUP_CONCAT(ii.ii_serialno SEPARATOR ', ') AS serial_numbers
    FROM orders o
    JOIN orders__item oi ON o.order_id = oi.order_id
    JOIN item i ON oi.item_id = i.item_id
    LEFT JOIN item_inventory ii ON oi.oi_id = ii.oi_id
    GROUP BY o.order_id, oi.oi_id, oi.oi_qty, i.item_name, i.item_price, oi.ip_override
);
-- -------------------------------------------------------------------
...
CREATE VIEW IF NOT EXISTS receipt_20_footer_subtotal AS (
    SELECT 
        r.order_id
      , r.order_date
      , SUM(r.quantity * r.item_price) AS subtotal
      , SUM(r.discount) AS total_discount
      , SUM((r.quantity * r.item_price) - r.discount) AS final_total
    FROM receipt_10_body_line_items AS r
    GROUP BY r.order_id
);
-- -------------------------------------------------------------------
...
CREATE VIEW IF NOT EXISTS receipt_21_footer_subtotal_taxes AS (
    SELECT
      ...
      , t.tax_type
      , t.tax_perc
      , (r.subtotal * t.tax_perc) / 100 AS tax_amount
    FROM receipt_20_footer_subtotal AS r  -- Alias for receipt_20_footer_subtotal
    CROSS JOIN taxes t
    WHERE r.order_date BETWEEN t.tax_beg AND COALESCE(t.tax_end, CURDATE())  -- Handle potentially null tax_end
);
-- -------------------------------------------------------------------
DROP VIEW IF EXISTS receipt_22_footer_grandtotal;

CREATE VIEW IF NOT EXISTS receipt_22_footer_grandtotal AS (
    SELECT
        r.order_id
      , SUM(r.subtotal) AS pre_tax_subtotal
      , SUM(r.tax_amount) AS total_tax
      , SUM(r.subtotal + r.tax_amount) AS grand_total
    FROM receipt_21_footer_subtotal_taxes AS r
    GROUP BY r.order_id
);
-- -------------------------------------------------------------------
